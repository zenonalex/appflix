...
void main() {
  ...
  setUp(() {
    ...
    getMovieCreditsUsecase = MockGetMovieCreditsUsecase();
  ...
  group('[GetMovieDetailsEvent] - ', () {
    blocTest(
      ...
      setUp: () {
        when(() => getMovieDetailsUsecase(movieId: any(named: 'movieId')))
    ...
    blocTest(
      ...
      ' when [GetMovieDetailsEvent] return a [Left]',
      setUp: () {
        when(() => getMovieDetailsUsecase(movieId: any(named: 'movieId'))).thenAnswer((_) async => Right(movie));
      },
      build: () => bloc,
      act: (MovieDetailsBloc bloc) => bloc.add(const GetMovieDetailsEvent(movieId: 640146)),
      expect: () => [
